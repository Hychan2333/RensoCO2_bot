...
__plugin_meta__ = PluginMetadata(
    name="QQ-Discord Relay",
    description="Relay messages between QQ groups and Discord channels.",
    usage="Configure DCQQ_RELAY_CHANNEL_LINKS in .env.dev.",
    type="application",
...
async def message_relay(
    ...
    if link is None:
        target_id = getattr(event, "group_id", None) or getattr(event, "channel_id", None)
        logger.debug(f"skip relay for unmapped target: {target_id}")
