...
class ReqResult(BaseModel):
    ...
    friend: list[FriendRequestResult] = Field(
        default_factory=list, description="好友请求列表"
    )
    group: list[GroupRequestResult] = Field(
        default_factory=list, description="群组请求列表"
    )
