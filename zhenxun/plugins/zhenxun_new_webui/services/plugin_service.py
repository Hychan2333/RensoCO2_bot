...
class PluginService:
    ...
    async def get_plugin_list(request: PluginListRequest) -> PluginListResult:
        ...
        for plugin in plugins:
            ...
            is_hidden = (
                plugin_type_enum == PluginType.HIDDEN if plugin_type_enum else False
            )
    ...
    async def toggle_plugin(request: PluginToggleRequest) -> bool:
        ...
        """
        plugin = await DbPluginInfo.get_plugin(module=request.module)
        if not plugin:
    ...
    async def get_plugin_config(module: str) -> PluginConfigResult:
        ...
        异常:
            NotFoundException: 插件不存在
        """
        plugin = await DbPluginInfo.get_plugin(module=module)
        if not plugin:
