...
router = WebUIRouter(prefix="/config", tags=["配置管理"])
...
async def save_yaml_file(user: AuthenticatedUser, content: str) -> APIResponse[bool]:
    """保存 config.yaml 配置文件"""
...
async def set_config_value(
    ...
    """设置指定模块的配置值"""
    result = ConfigService.set_config_value(request.module, request.key, request.value)
